...
class HSL_Layer_Part2(nn.Module):
    """Extension: Incorporate temporal information into phenotype extraction"""
    ...
    def __init__(self, n_c, emb_dim, add_ratio, temperature):
        ...
        # NEW: Learnable time decay parameter
        self.time_decay = nn.Parameter(torch.tensor(0.1))
    
    def forward(self, X, H, V, E, incident_mask_prob, time_gaps=None):
        ...
        # NEW: Apply temporal weighting if time gaps provided
        if time_gaps is not None:
            # time_gaps: [num_visits] - days since each visit
            # More recent visits (smaller gaps) get higher weights
            time_weights = torch.exp(-F.softplus(self.time_decay) * time_gaps)
            time_weights = time_weights / time_weights.sum()  # Normalize
            S = S * time_weights.unsqueeze(0)  # [num_codes, num_visits]
        
        # Rest is same as original
        S[V, E] = -1e30
        k = max(1, int(self.add_ratio * E.shape[0]))
...
class HSLEncoder(nn.Module):
    def __init__(self, code_dims, HGNN_dim, after_HGNN_dim, HGNN_layer_num, nhead, num_TP, temperature, add_ratio, n_c, hid_state_dim, dropout, HGNN_model, device):
        ...
        # CHANGED: Use time-aware version
    ...
    # CHANGED: Added time_gaps parameter
    def forward(self, X, H, time_gaps=None):
        ...
        if self.num_TP > 1:
            ...
            # CHANGED: Pass time_gaps to HSL_P2
            TPs = torch.stack([self.HSL_P2_combo[k](X_1, H, V, E, incident_mask_probs[k], time_gaps) for k in range(self.num_TP)])
            ...
            # CHANGED: Pass time_gaps to HSL_P2
            TPs = self.HSL_P2_combo[0](X_1, H, V, E, incident_mask_probs, time_gaps)
...
class SHy(nn.Module):
    ...
    # CHANGED: Added time_gaps_list parameter
    def forward(self, Hs, visit_lens, time_gaps_list=None):
        # Hierarchical embedding for medical codes.
        ...
        for i in range(len(Hs)):
            # CHANGED: Get time_gaps for this patient (or None if not provided)
            time_gaps_i = time_gaps_list[i] if time_gaps_list is not None else None
            
            # CHANGED: Pass time_gaps to encoder
            tp, latent_tp, _ = self.encoder(X, Hs[i][:, 0:int(visit_lens[i])], time_gaps_i)
            tp_list.append(tp)
