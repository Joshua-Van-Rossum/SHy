...
if __name__ == '__main__':
    ...
    parser.add_argument('--single_dim', type=int, default=4, help="embedding dimension of one ICD-9 level")
    parser.add_argument('--HGNN_dim', type=int, default=16, help="hidden dim in HGNN")
    parser.add_argument('--after_HGNN_dim', type=int, default=8, help="hidden dim after HGNN")
    parser.add_argument('--HGNN_layer_num', type=int, default=1, help="number of HGNN layers")
    parser.add_argument('--nhead', type=int, default=2, help="number of heads in HGNN")
    parser.add_argument('--num_TP', type=int, default=3, help="number of temporal phenotypes")
    parser.add_argument('--n_c', type=int, default=2, help="number of cosine weight vectors")
    parser.add_argument('--hid_state_dim', type=int, default=16, help="temporal phenotype embedding dim")
    parser.add_argument('--key_dim', type=int, default=16,  help="key dim for self attention")
    parser.add_argument('--SA_head', type=int, default=4,  help="number of heads for self-attention")
    ...
    parser.add_argument('--extension', type=int, default=0, help="Use model with extension. Only for MIMIC-IV")
    ...
    if args.extension == 1:
        print("Using extension training and model.")
        from extension_model import SHy
        from extension_training import train
    else:
        print("Using standard training and model.")
    ...
    if args.dataset_name == 'MIMIC_III':
        ...
        ## Extension, time gaps
        if args.extension == 1:
            with open(f'{data_dir}/train_time_gaps.pkl', 'rb') as f:
                train_time_gaps = pickle.load(f)
            with open(f'{data_dir}/test_time_gaps.pkl', 'rb') as f:
                test_time_gaps = pickle.load(f)
    ...
    if args.extension == 1:
        model = SHy(
            code_levels, args.single_dim, args.HGNN_dim, args.after_HGNN_dim, 
            args.HGNN_layer_num-1, args.nhead, args.num_TP, args.temperature,
            args.add_ratio, args.n_c, args.hid_state_dim, args.dropout, 
            args.key_dim, args.SA_head, args.HGNN_model, device
        ).to(device)
    else:
    ...
    if args.dataset_name == 'MIMIC_III':
        ...
        train_loader = DataLoader(training_data, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=True)
        ...
        train_loader = DataLoader(training_data, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=4)
        test_data = MIMICivDataset('binary_test_x_slices/binary_test_codes_x', test_visit_lens, trans_y_test, test_pids, 'Test')
        test_loader = DataLoader(test_data, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=4)
    ...
    if args.extension == 1:
        r2_list, r4_list, n2_list, n4_list, test_loss_per_epoch, train_average_loss_per_epoch, prediction_loss_per_epoch = train(
            ...
            train_time_gaps=train_time_gaps,
            test_time_gaps=test_time_gaps,
